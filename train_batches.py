# @Author: chenleli
# @Date:   2018-7-13 15:54:17
# @Email:  chenle.li@student.ecp.fr
# @Filename: train_batches.py
# @Last modified by:   chenleli
# @Last modified time: 2018-7-14 09:54:51
...
from utils.tf_record.py import tf_record
...
class Solver(object):
    ...
    def train_queue_method():
        """
        Given the huge amount of data in OpenImage4, considering use
        queue to save the usage of memory.
        """
        ...
        for step in range(1, self.max_iter + 1):
            ...
            if step % self.summary_iter == 0:
                if step % (self.summary_iter * 10) == 0:
                    ...
                    # TODO check for queque method about two sess call
                    summary_str, loss, _ = self.sess.run(
                        [self.summary_op, self.net.total_loss, self.train_op],
                        feed_dict={x_place:image_batch,y_place:label_batch})
